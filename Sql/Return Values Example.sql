...
USE [Utilities];
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('tc.dd_TruncateCollectionsEntriesYieldingEligInfoToday'))
   DROP PROC [tc].[dd_TruncateCollectionsEntriesYieldingEligInfoToday]
GO
CREATE PROCEDURE [tc].[dd_TruncateCollectionsEntriesYieldingEligInfoToday]
/* -----------------------------------------------------------------------------------------------------------
   Procedure Name  :  tc.dd_TruncateCollectionsEntriesYieldingEligInfoToday
   Business Analyis:
   Project/Process :   
   Description     :  Truncate CollectionsEntriesYieldingEligInfoToday table.
...
--  Initialize Batch
 ...
 ----------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SET @BatchDetailDescription = 'Truncate CollectionsEntriesYieldingEligInfoToday'
    ...
    -- CollectionsEntriesYieldingEligInfoToday
    TRUNCATE TABLE [TennCareEligibility].[tc].[CollectionsEntriesYieldingEligInfoToday];
