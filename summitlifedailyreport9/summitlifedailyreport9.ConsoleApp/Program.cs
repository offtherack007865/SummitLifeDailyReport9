...
using summitlifedailyreport9.ConsoleApp;
...
namespace summitlifedailyreport9.AmassInputDataAndCreateExcelConsoleApp
{
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            IConfiguration config = new ConfigurationBuilder()
                .AddJsonFile(summitlifedailyreport9.Data.MyConstants.AppSettingsFile)
                .Build();

            // Read in Configuration Options for this Console Application

            summitlifedailyreport9.ConsoleApp.ReadInConfigOptions myReadInConfigOptions = 
                new summitlifedailyreport9.ConsoleApp.ReadInConfigOptions(config);
            ...
            qy_GetDailyReportConfigOutput
                myqy_GetDailyReportConfigOutput =
                    myCallForGetOptions
                    .qy_GetDailyReportConfig();
            if (!myqy_GetDailyReportConfigOutput.IsOk ||
                myqy_GetDailyReportConfigOutput.qy_GetDailyReportConfigOutputColumnsList.Count != 1)
            {
                log.Error($"We had an error in trying to get the configuration file from the database:  {myqy_GetDailyReportConfigOutput.ErrorMessage}");
                return;
            }
            ...
            DailyReportsMainOps
                myMainOps =
                    new DailyReportsMainOps
                    (
                        myConfigOptions
                        , myqy_GetDailyReportConfigOutput.qy_GetDailyReportConfigOutputColumnsList[0]
                    );
            ...
            {
                string mySubjectLine = "Summit Life Daily Reports were created for today.";
                string myBody = string.Join("<br>", myMainOutput.MailBodyLineList);   //myMainOutput.EmailBody;

                string[] myEmailAddressArray =
                    myqy_GetDailyReportConfigOutput
                    .qy_GetDailyReportConfigOutputColumnsList[0]
                    .Emailees
                    .Split(";");
